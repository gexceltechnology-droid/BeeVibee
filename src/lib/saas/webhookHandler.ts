  ...
  static async processWebhook(
    ...
    try {
      ...
      if (!isAuthoritativeEvent) {
        ...
        await client.query('COMMIT;');
        return {
          statusCode: 200, success: true, ignored: true, message: `Event type ${normalized.eventType} is recorded as IGNORED without financial mutation.`, reason: `Event type ${normalized.eventType} is recorded as IGNORED without financial mutation.`,
        };
      }
      ...
      if (orderRes.rows.length === 0) {
        ...
        await client.query('COMMIT;');
        return {
          statusCode: 200, success: true, ignored: true, message: `No corresponding payment_orders record found for provider order reference: ${normalized.providerOrderId}`, reason: `No corresponding payment_orders record found for provider order reference: ${normalized.providerOrderId}`,
        };
      }
